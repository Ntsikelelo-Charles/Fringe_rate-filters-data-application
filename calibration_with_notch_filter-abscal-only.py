...
def main():
    ...
    hd = io.HERAData(path_data+"filtered_single_pol_combined_"+filter_name+"_data_"+lst+"_"+spw+".uvh5")
    ...
    for x in range (N): 
        ...
        hd_temp.write_uvh5("/net/sinatra/vault-ike/ntsikelelo/filtered_data/abscal_only_filtered_data_"+uvh5name[i][43:69], clobber=True)
    ...
    for x in range (N): 
        ...
        hd_filt = io.HERAData("/net/sinatra/vault-ike/ntsikelelo/filtered_data/abscal_only_filtered_data_"+uvh5name[i][43:69])
        ...
        times=hd.times[0:2]
        freq_range=hd.freq_array ### might need to look this again
        
        data, _, _ = hd.read(frequencies=freq_range, times=times)
        diff_data, _, _ = hd_diff.read(frequencies=freq_range, times=times)
        data_filt, _, _ = hd_filt.read(times=times[0]) #not setting the requency range yet
        ...
        sol, meta, overall_class, vissol_flags=redundant_calibration (data, redcal_class, overall_class, reds, rfi_flags)  
        
        hd.update(data=sol.vis)
        hd.write_uvh5(path_data+"redundant_Abscal_only_"+filter_name+"_cal_data_temp.uvh5", clobber=True)
        ...
        def produce_model_data(mode="full_sky_model", freqs=freq_range, times=times):
            hdm_gleam = io.HERAData(path_data+"Filtered_model_"+filter_name+"_gleam_vis_expanded_"+lst+"_"+spw+".uvh5")
            hdm_gleam.read(times=times, frequencies=freqs)
            model_data_gleam,_,_=hdm_gleam.build_datacontainers()
            
            if mode=="full_sky_model":
                hdm_diffuse = io.HERAData(path_data+"Filtered_model_"+filter_name+"_diffuse_vis_expanded_"+lst+"_"+spw+".uvh5")
            ...
            return model_data 
        hdm_gleam = io.HERAData("/net/sinatra/vault-ike/ntsikelelo/Data_H6C/Filtered_model_"+filter_name+"_gleam_vis_expanded_"+lst+"_"+spw+".uvh5")
        ...
        hd_cal= io.HERAData(path_data+"redundant_Abscal_only_"+filter_name+"_cal_data_temp.uvh5")
        hd_cal.read(bls=matched_bls, frequencies=freq_range_reduced, times=times[0])
        redcal_data_temp,_,_=hd_cal.build_datacontainers()        
        
        redcal_gains={}
        for key in sol.gains:
            gains_temp=np.zeros(shape=(1,205), dtype=complex)
            gains_temp[0,:]=sol.gains[key][0,:][indexfreq]
            redcal_gains[key]=gains_temp
            # print(key, redcal_gains[key].shape)
            
        redcal_data=copy.deepcopy(redcal_data_temp)
        for key in redcal_data:
            redcal_data[key]=data_filt[key]
            # print(key, redcal_data[key].shape)
        abscal.calibrate_in_place(redcal_data, redcal_gains)
            ...
            if filter_name=="Main_lobe_20_mHz" or filter_name=="Main_lobe_baseline_dependent":
                ...
                for k in noise_wgts:
                    if k not in filter_center_all:
                        ...
                        n_include+=1
                print("number of baseline include is "+str(n_include)) 
            else:    
                for k in noise_wgts:
        ...
        redcal_gains={}
        for key in sol.gains:
            redcal_gains[key]=sol.gains[key][0,indexfreq[0]-1:indexfreq[-1]]   
        ...
        np.save(path_data+"gains/gains_Abscal_only_"+filter_name+"_gleam_flagged_abscal"+uvh5name[i][43:69]+"_"+lst+"_"+spw+".npy",total_gains_flagged_gleam)
        np.save(path_data+"gains/gains_Abscal_only_"+filter_name+"_full_flagged_abscal"+uvh5name[i][43:69]+"_"+lst+"_"+spw+".npy",total_gains_flagged_full)
        
        np.save(path_data+"gains/gains_Abscal_only_"+filter_name+"_gleam_abscal"+uvh5name[i][43:69]+"_"+lst+"_"+spw+".npy",total_gains_gleam)
        np.save(path_data+"gains/gains_Abscal_only_"+filter_name+"_full_abscal"+uvh5name[i][43:69]+"_"+lst+"_"+spw+".npy",total_gains_full)
        ...
        np.save(path_data+"abs_calibrated_data_Abscal_only_"+filter_name+"_full_"+uvh5name[i][43:69]+"_"+lst+"_"+spw+".npy", cal_full)
        np.save(path_data+"abs_calibrated_data_Abscal_only_"+filter_name+"_full_filtered_"+uvh5name[i][43:69]+"_"+lst+"_"+spw+".npy", cal_full_filtered)
        
        np.save(path_data+"abs_calibrated_data_Abscal_only_"+filter_name+"_gleam_"+uvh5name[i][43:69]+"_"+lst+"_"+spw+".npy", cal_gleam)
        np.save(path_data+"abs_calibrated_data_Abscal_only_"+filter_name+"_gleam_filtered_"+uvh5name[i][43:69]+"_"+lst+"_"+spw+".npy", cal_gleam_filtered)
