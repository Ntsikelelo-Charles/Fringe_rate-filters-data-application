...
def main():
    ...
    for x in range (N): 
        ...
        def produce_model_data(mode="full_sky_model", freqs=freq_range, times=times):
            hdm_gleam = io.HERAData(path_data+"Model_gleam_vis_expanded_"+lsts_center+"_"+spw+".uvh5")
            hdm_gleam.read(times=times, frequencies=freqs)
            model_data_gleam,_,_=hdm_gleam.build_datacontainers()
            if mode=="full_sky_model":
        
                hdm_diffuse = io.HERAData(path_data+"Model_diffuse_vis_expanded_"+lsts_center+"_"+spw+".uvh5")
        ...
        hdm_gleam = io.HERAData("/net/sinatra/vault-ike/ntsikelelo/Data_H6C/Model_gleam_vis_expanded_"+lsts_center+"_"+spw+".uvh5")
        ...
        np.save(path_data+"gains/gains_gleam_flagged_abscal"+uvh5name[i][43:69]+"_"+lsts_center+"_"+spw+".npy",total_gains_flagged_gleam)
        np.save(path_data+"gains/gains_full_flagged_abscal"+uvh5name[i][43:69]+"_"+lsts_center+"_"+spw+".npy",total_gains_flagged_full)
        
        np.save(path_data+"gains/gains_gleam_abscal"+uvh5name[i][43:69]+"_"+lsts_center+"_"+spw+".npy",total_gains_gleam)
        np.save(path_data+"gains/gains_full_abscal"+uvh5name[i][43:69]+"_"+lsts_center+"_"+spw+".npy",total_gains_full)
        ...
        np.save(path_data+"abs_calibrated_data_full_"+uvh5name[i][43:69]+"_"+lsts_center+"_"+spw+".npy",cal_full)
        np.save(path_data+"abs_calibrated_data_gleam_"+uvh5name[i][43:69]+"_"+lsts_center+"_"+spw+".npy", cal_gleam)
